...
for FILENAME in $(find ../collections -iname "*.postman_*.json"); do
    echo -n "Removed "
    perl -0777 -p -i -e '$k+= s/("key": "(?:password|username|Authorization|value)",\s*"value": ")(?:\s*[^{]{2,2}.+[^}]{2,2}\s*)(",\s*"type": ".*")/\1\2/g; END{print "$k"}' $FILENAME
...
for FILENAME in $(find ../environments -iname "*.postman_*.json"); do
    echo -n "Removed "
    perl -0777 -p -i -e '$k+= s/("key": ".+",\s*"value": ").+(",\s*"type": "secret")/\1\2/g; END{print "$k"}' $FILENAME
    echo " secrets from \"$FILENAME\""
done
